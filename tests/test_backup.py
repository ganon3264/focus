...
class TestBackupFunctions:
    async def test_create_and_list_backup(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = os.path.join(tmp, "focus.db")
            os.makedirs(os.path.join(tmp, "assets", "characters"), exist_ok=True)
    ...
    async def test_list_backups_empty(self):
        with tempfile.TemporaryDirectory() as tmp:
            backups_path = os.path.join(tmp, "empty_backups")
            backups = list_backups(backups_path=backups_path)
            assert backups == []
    ...
    async def test_restore_imports_data(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = os.path.join(tmp, "focus.db")
            os.makedirs(os.path.join(tmp, "assets"), exist_ok=True)
            backups_path = os.path.join(tmp, "backups")
    ...
    async def test_delete_nonexistent_backup(self):
        with tempfile.TemporaryDirectory() as tmp:
            backups_path = os.path.join(tmp, "empty_backups")
            with pytest.raises(FileNotFoundError):
